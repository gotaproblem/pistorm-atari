...
if "%CPLD%"=="EPM240" (
%quartus_bin_path%\quartus_map %PROJECT_NAME%%CPLD%
goto :compile
)
if "%CPLD%"=="EPM570" (
%quartus_bin_path%\quartus_map %PROJECT_NAME%%CPLD%
...
%quartus_bin_path%\quartus_sh --flow compile %PROJECT_NAME%%CPLD%
if %errorlevel% neq 0 GOTO ERRORCOMPILE

%quartus_bin_path%\quartus_cpf -c -q 100KHz -g 3.3 -n p output_files/%PROJECT_NAME%%CPLD%.pof %BITSTREAM%.svf
