...
alloc_mapram:
      ...
      /* cryptodad */
      case MAPTYPE_FILE:
//printf ("[SXB] filename = %s\n", filename );
        in = fopen ( filename, "rb" );

        if ( !in )
        { 
          printf ( "[CFG] Failed to open file %s for disk mapping\n", filename );
        ...
        //if (size == 0) 
        //{
          cfg->map_size[index] = file_size + headersz;
          cfg->map_high[index] = addr + cfg->map_size[index] + headersz;
        //}
//printf ( "[SXB] filename %s is %d bytes\n", filename, file_size );
        fseek (in, 0, SEEK_SET);

        cfg->map_data[index] = (unsigned char *)calloc (1, cfg->map_size[index]);
        cfg->image_size[index] = file_size; //(cfg->map_size[index] <= file_size) ? cfg->map_size[index] : file_size;

        if (!cfg->map_data[index]) 
        {
          printf ("[CFG] ERROR: Unable to allocate memory for mapped disk image!\n");
        ...
        *((unsigned int*)cfg->map_data[index]) = be32toh (file_size); /* write filesize to disk image header */

        err = fread (cfg->map_data[index] + headersz, cfg->image_size[index], 1, in);

        if ( err == 0 )
        {
          printf ( "[CFG] Failed to copy disk image to mapped RAM %d\n", err );
...
int get_mapped_item_by_address(struct emulator_config *cfg, uint32_t address) {
  for (int i = 0; i < MAX_NUM_MAPPED_ITEMS; i++) {
    if (cfg->map_type[i] == MAPTYPE_NONE || !cfg->map_data[i])
      ...
      if (cfg->map_type[i] == MAPTYPE_RAM || cfg->map_type[i] == MAPTYPE_RAM_NOALLOC || cfg->map_type[i] == MAPTYPE_ROM || cfg->map_type[i] == MAPTYPE_FILE)
        return i;
    }
  }
